...
class FileRepository(private val responseFactory: PagedResponseFactory<File> = DefaultResponseFactory()) {
   ...
   */
  fun findAll(pageable: Pageable = Pageable()): PagedResponse<File> {
    ...
    try {
      ...
      return PagedResponse(pageable.page, pageable.maxSize, 0, emptyList())
    } catch (e: Exception) {
   ...
   */
  fun findById(id: Long): Optional<File> {
    val session = this.databaseSessionBuilder.build()
   ...
   * @throws RuntimeException if an error happened during the search
   */
  fun findByName(name: String, pageable: Pageable = Pageable()): PagedResponse<File> {
    val session = this.databaseSessionBuilder.build()
   ...
   * @throws RuntimeException if the persistance fails at runtime
   */
  fun persistFile(vararg files: File) {
    val databaseSession = this.databaseSessionBuilder.build()
    val transaction = databaseSession.manager.transaction

    try {
      transaction.begin()
      files.forEach { databaseSession.manager.persist(it) }
      transaction.commit()
   ...
   */
  fun updateFile(vararg files: File) {
    val databaseSession = this.databaseSessionBuilder.build()
    val transaction = databaseSession.manager.transaction

    try {
      transaction.begin()
      files.forEach { databaseSession.manager.merge(it) }
      transaction.commit()
    } catch (e: Exception) {
      this.rollbackTransactionIfActive(transaction)
   ...
   */
  fun removeFile(vararg files: File) {
    val databaseSession = this.databaseSessionBuilder.build()
    val transaction = databaseSession.manager.transaction
    ...
    try {
      transaction.begin()
      files.forEach {
        databaseSession.manager.remove(databaseSession.manager.find(File::class.java, it.id))
      }
      transaction.commit()
    } catch(e: Exception) {
      this.rollbackTransactionIfActive(transaction)
